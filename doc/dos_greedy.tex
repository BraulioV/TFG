...
Una de las simplificaciones más importantes que se ha hecho del modelo es la eliminación total de los materiales que tiene cada aula y que necesita cada asignatura. Esto simplifica, aunque pueda parecer muy simple, simplifica enormemente el modelo. Pero esta modificación no se hace así sin más, sino que en vez de haber una lista de materiales posibles y necesitados y buscar posibles aulas haciendo una intersección, se cambia esto por una lista de aulas en las que se puede impartir una asignatura y se elige entre estas. A continuación, en la \hyperref[clases3]{Figura \ref*{clases3}}, se puede ver un nuevo diagrama de clases conceptual en el que se introduce la lista de posibles aulas y se eliminan los materiales.

\begin{figure}
  \input{clases3.tex}
  \caption{Nuevo diagrama de clases conceptual.}
  \label{clases3}
\end{figure}

Esto puede parecer un poco simple, pero añade mucha flexibilidad al modelo, ya que podemos guiar el espacio de búsqueda para las aulas de prácticas restringiendo las aulas que pueden tomar ciertas asignaturas y cuales no, lo que permite una flexibilidad enorme. Además de esto, controlar las aulas más problemáticas como las que necesita \textit{Fundamentos de Redes} o \textit{Tecnología y Organización de los Computadores}, que son aulas muy específicas, es tan sencillo como limitar las aulas posibles a esas aulas, y ya el sistema se encarga de asignarlo de forma eficiente.
...
% En primer lugar, calcula las horas de teoría y de prácticas que tiene cada curso y, después, calcula el número total de horas de prácticas que hay en los laboratorios. 
